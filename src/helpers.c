...
static int create_state_dirs(const char *statedir_path)
{
	int ret = 0;
	unsigned int i;
	char *dir;
#define STATE_DIR_COUNT (sizeof(state_dirs) / sizeof(state_dirs[0]))
	...
	// check for existence
	if (ensure_root_owned_dir(statedir_path)) {
		//state dir doesn't exist
		if (mkdir_p(statedir_path) != 0 || chmod(statedir_path, S_IRWXU) != 0) {
	...
	for (i = 0; i < STATE_DIR_COUNT; i++) {
		string_or_die(&dir, "%s/%s", statedir_path, state_dirs[i]);
	...
	return ensure_root_owned_dir(statedir_path);
}
